...
| 사용 도구 | Make, n8n |
| Trigger | Schedule Google Spread Sheet — 15분마다 새로운 응답 감지 |
| 조건 분기 | 점수 기준 Router(Make) / if(n8n) |
| 주요 Action | Google Gemini 채점/답변생성,Google Sheets 행 추가, Gmail 메일 발송 |

### 프로젝트 2 — 웹훅 트리거와 rssFeed를 활용한 게시물 분류 요약 저장

| 구분 | 내용 |
| --- | --- |
| 반복 업무 | Tally(웹폼)으로 접수된 게시물과 Blogger(블로그)에 게시된 게시물들을 수집하여 내용을 분류 요약하고 DB(notion)에 저장한다. |
| 선정 도구 | n8n |
| 선정 이유 | 자체 호스팅가능하여 비용을 절약 할 수 있고 업무단위 분리가 수월하다. |
| 흐름 | Trigger → 대상 모음 → 조건 분기 → 컨텐츠처리 혹은 오류처리 → 조건 분기 → DB저장 |
