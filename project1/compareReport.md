...
|비교항목 | Make | n8n | 
| --- | --- | --- |
| 1 UI/UX| 둘다 시각적 node 기반 사용성은 비슷하지만make가 비전문적인 작업에 조금더 편리|단 이전단계를 실행시켜가면서 하면 n8n이 더 자세한 내용을 볼 수 있다.| 
| 2 설정난이도 | 동일한 기능에 한하여 make가 조금더 직ㅎ하ㅎㅎ n8n이 다건처리등 복잡한 기능 구현에는 더 간편함  | 
| 3 연동 서비스 범위 | make가 googleForm을 직접 제공하는 등 조금 더 많음| make 보다 google Form등 일부 연동 모듈이 적음 | 
| 4 무료플랜 | 매달 1000 크레딧 (core요금제 기준 1크레딧당 $0.0009) |영구적인 무료플랜 제공하지 않으나 자체호스팅 가능 이 경우 셀프 호스팅 비용 만큼으로 줄일 수 있음| 
| 5 업무단위 분리,스케줄 | 다건도 제공하나 단건 기준으로 진행되는 부분이 많음 다건 처리를 위해 별도의 조치가 필요 반복 스케줄이 타이머기반 | 다건의 처리를 자동으로 해주는 부분이 많음 반복 스케줄이 크론 기반 | 

## 도구 장단점

make는 단건처리나 단순한 과제를 수행하는데 있어서는 직관적인 UI와 낮은 설정난이도를 보인다. 하지만 다건처리에 있어서는 n8n보다 설정난이도가 높아지는 경향이 있었다.
n8n은 자체 호스팅시 비용을 훨씬더 절감 할 수 있고 다건처리를 자동으로 해주는 노드가 많았다. 전반적으로 조금 더 복잡한 기능을 구현하는데 유리한 경향이 있다.

## 상황별 적합도 의견

단건중심 테스크나 간편한 구현에는 make의 UI/UX가 더 편리 하고 직관적이지만,
복잡한 테스크나 다건동시처리에는 n8n을 사용하는 것이 더 유리하다.

또한 n8n은 호스팅이 가능하다면 비용면에서 절대적인 우위를 가질 수 있다.

스케줄링 cron형 반복에는 n8n이 기본기능으로 제공해줘서 편리함
단 단순 주기로 반복되는 타이머는 make의 기본 기능으로 구현하는것이 훨씬 쉬움
